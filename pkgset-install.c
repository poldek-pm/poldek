...
static
int do_select_best_pkg(const struct pkg *marker,
                       struct pkg **candidates, int npkgs,
                       struct pkgset *ps, struct upgrade_s *upg);

static
int select_best_pkg(const struct pkg *marker, tn_array *candidates,
                    struct pkgset *ps, struct upgrade_s *upg);
...
static
int is_pkg_installed(struct pkgdb *db, struct pkg *pkg, int *cmprc) 
{
    tn_array *dbpkgs = NULL;
    int n;
    
    n = pkgdb_search(db, &dbpkgs, PMTAG_NAME, pkg->name, NULL, PKG_LDNEVR);
...
int is_installable(struct pkg *pkg, struct poldek_ts *ts, int is_hand_marked) 
{
    ...
    npkgs = is_pkg_installed(ts->db, pkg, &cmprc);
                ...
                struct pkgset *ps, struct upgrade_s *upg, int nobest)
{
    ...
    *best_pkg = NULL;
    ...
    if (found && nsuspkgs) {
        struct pkg **matches, **new_suspkgs;
        int n, i, nmatches = 0;

        /* remove marked for removal items from suspected packages */
        new_suspkgs = alloca(sizeof(*new_suspkgs) * nsuspkgs);
        n = 0;
        for (i=0; i < nsuspkgs; i++) {
            if (marked_for_removal(suspkgs[i], upg)) {
        ...
        if (n == 0)             /* all packages are marked for removal */
            goto l_end;
        ...
        if (psreq_match_pkgs(pkg, req, upg->strict, suspkgs,
            ...
            if (nmatches > 0 && !one_is_marked(upg->ts->pms, matches,
                ...
                if (nobest == 0 && nmatches > 1)
                    best_i = do_select_best_pkg(pkg, matches, nmatches, ps, upg);
                      ...
                      struct upgrade_s *upg)
{
    ...
    n = get_obsoletedby_pkg(db, upg->uninst_set->dbpkgs, pkg, getflags,
                     ...
                     struct upgrade_s *upg, int process_as) 
{
	...
	DBGF("%s, ndrags %d\n", pkg_id(pkg), ndrags);
        if (ndrags == 0 || 1) { /* XXX cond temporary disabled - needs test */
            ...
            if (p && (pkg_is_marked(upg->ts->pms, p) ||
                      pkg_is_marked_i(upg->ts->pms, p)))
                is_marked = 1;
...
int find_replacement(struct upgrade_s *upg, struct pkg *pkg, struct pkg **rpkg)
{
    ...
    *rpkg = NULL;
    ...
    } else if ((pkgs = pkgset_search(upg->ts->ctx->ps, PS_SEARCH_OBSL, pkg->name))) {
        int i;
        
        for (i=0; i < n_array_size(pkgs); i++) {
            struct pkg *p = n_array_nth(pkgs, i);
            if (pkg_caps_obsoletes_pkg_caps(p, pkg) &&
                pkg_cmp_name_evr(p, pkg) > 0) {
                *rpkg = p;
                break;
            }
        }
 ...
 l_end:
    if (is_cnfl) {
        logn(LOGERR, _("%s: /%s/%s%s: conflicts with %s's one"),
            pkg_id(pkg),
            dirname, flfile->basename,
            S_ISDIR(flfile->mode) ? "/" : "", dbpkg_id(dbpkg));
            
    } else if (verbose > 1) {
        msg(3, "/%s/%s%s: shared between %s and %s\n",
            dirname, flfile->basename,
            S_ISDIR(flfile->mode) ? "/" : "",
            pkg_id(pkg), dbpkg_id(dbpkg));
    }
        ...
        if (pkg_match_req(dbpkg, cnfl, 1)) {
            if (!resolve_conflict(indent, pkg, cnfl, dbpkg, ps, upg)) {
                logn(LOGERR, _("%s (cnfl %s) conflicts with installed %s"),
                    pkg_id(pkg), capreq_snprintf_s(cnfl),
                    pkg_id(dbpkg));
                ncnfl++;
            }
        }
                        ...
                        int vrfy)
{
    ...
    for (i=0; i<n_array_size(upg->install_pkgs); i++) {
        ...
        if (vrfy) {
            ...
            is_installed = is_pkg_installed(upg->ts->db, pkg, &cmprc);
