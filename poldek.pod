...
The source is specified by B<--source> option:
...
=item * as packages names with:

B<-n, --pkgnevr="NAME [[E:][V[-R]]]">

for example:

$ poldek ... -n "apache 1.3.12" -n gzip 

$ poldek ... -n "Bezdany 1:908.9-26" -n ziptool
...
AFAIK rpm 3.x doesn't handle this case (4.x introduces special 
tag RPMTAG_MULTILIB for these cases, I suppose), so poldek
does not handle it also.  
...
I<<pld-installer@pld.org.pl>> or to the author.
