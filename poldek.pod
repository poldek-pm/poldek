...
This option verifies dependenecies of package set.
...
option and additioanlly from $HOME/.poldek_hold. Held package won't be upgraded 
with exeption if it will be pointed directly by the user.
...
Don't take held packages from $HOME/.poldek_hold nor config's hold paramater.
...
=item * --test (may be given twice, see above --test description)
...
then it should be specified by suffixed paramteters pair to distinguish 
that source from other ones:   
...
=item * B<use_sudo = yes|no> - for r/w operations poldek will 
        run rpm using sudo. The default is "no", but is is recommended
        to change it to "yes" and use the poldek as ordinary user.
