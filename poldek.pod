...
of rpm packages. It can be used in two modes: batch and interactive mode. 
It was inspired by L<apt-get(8)> from Debian's APT, the interactive mode 
was inspired by shell mode of Perl CPAN module ($ perl -MCPAN -e shell). 
...
If you want to build package index, then only one and only directory
path as SOURCE is accepted.

If package index and packages are stored in diffrent
...
Additionally previous SOURCE/packages.dir[.gz] is different from 
currently created one then index "patch" is created 
under SOURCE/packages.i/ directory. Every patch consist of two 
data file: packages.dir.diff.YYYY.MM.DD-HH.mm.SS.gz and 
digest file packages.dir.diff.YYYY.MM.DD-HH.mm.SS.mdd. 
...
For local sources option only verifies existing index 
...
If option is not given packages to hold is taken from B<hold> configuration 
option and $HOME/.poldek_hold. Held package won't be upgraded with exeption
if it be pointed directly by the user.
...
the default is $HOME/.poldek-cache; if set $TMPDIR is used.

=item B<--log=FILE>

Log all poldek's messages to FILE
...
=item * --test (if given twice, see above --test description)
...
Program tries to read configuration from F</etc/poldek.conf> by default,
F<$HOME/.poldekrc> is used instead if it exists. B<-c,--conf> option could points to 
other location of config. With B<--noconf> poldek don't read any config. 
...
=item * B<source> = [NAME;]SOURCE

Optional NAME may be specified later from cmdline by
B<-n, --sn> B<SOURCE-NAME> option; for example if you 
have 3 sources configured as follows:

$ ./poldek -l
pld-devel     /mnt/PLD-stable/RPMS
pld-stable    ftp://zn/PLD/i686/PLD/RPMS
misRPMS       ftp://zn/rpm/RPMS/

then to load only ftp://zn/PLD/i686/PLD/RPMS use:

$ poldek --sn pld-stable 

..and to load all two pld-* use:

$ poldek --sn pld-\*

If you want to configure prefixed source, then it should be specified 
by suffixed source/prefix pair to distinguish it from other sources.   

=item * B<sourceXX> = [NAME;]SOURCE
=item * B<prefixXX> - see B<-P> description
...
=item * B<hold = PACKAGE[ PACKAGE...]> - see --hold option description

=item * B<mercy = yes|no> - see --mercy option description

=item * B<keep_downloads = yes|no> - by default poldek removes downloaded
        packages files after successfull installation; if option is set to 
        "yes" then program will never remove downloaded packages from cache 
        directory.
...
$ poldek -s /cdrom/PLD/RPMS --mkidx=/tmp/packages.dir.gz
...
watchfull. Remember, that there is I<NO WARRANTY OF ANY KIND> and enjoy.
