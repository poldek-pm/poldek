...
source = ~/tmp/my-cd-indexes/

prefix = /mnt/cdrom/RPMS/

=item * B<cachedir> - see B<--cachedir> option description

=item * B<use_sudo = yes|no> - for r/w operations poldek will 
        run rpm using sudo. The default is "no", but it is recommended
...
=item * B<confirm_installs = yes|no> - if "yes" program confirm all
        performed package installations (default: no)

=item * B<choose_equivalents_manually = yes|no> - if "yes" program let the user
        choose among equivalent packages (default: no)
        
=item * B<particle_install = yes|no> - if "no" installations of multiple packages 
        are performed as one transaction (default: yes)
=back
