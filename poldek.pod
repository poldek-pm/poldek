...
of rpm packages. It can be used in two ways: as cmdline or as 
interactive tool. It was inspired by L<apt_get(8)> tool from Debian's 
...
If you want to build Packages, then only one and only directory
...
$ poldek -s /tmp/Packages.gz --prefix=/cdrom/RPMS/ -s ...
...
digest and packages.dir-toc with packages NAME-EVR per line.  
I<NOTE>: SOURCE must be directory.

=item B<--update>    

Resynchronize the package index from its source.
It is necessary only for remote packages.dir and it 
...
=item B<--split=[SIZE] [--split-conf=FILE] [--split-out=PREFIX]>

Split package set into parts, each SIZE MB size. Priorities 
are taken from FILE given by --split-conf (see sample poldek-split.conf 
in distribution). By default packages has 0 priority.

Each part is written as NAME-EVR list to PREFIX.XX 
file, the default PREFIX is "package.chunk"
...
$ poldek -s ftp://ftp.rankiem.net.pl/PLD/RPMS/packages.dir 
