...
B<OPTIONS> describe source options, for now only 
'noauto' (don't load this source by default) is accepted, e.g.
...
=item * B<keep_downloads = yes|no> 
        ...
        Program removes downloaded packages files after successfull installation
