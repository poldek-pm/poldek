...
itself or by external downloaders (not configured by default, 
see L</CONFIG FILE>). 
...
index file is used; with B<--sdir> desired directory is scanned 
instead of reading index file, should be used for local directories only. 
The last one B<--shdrl> can be used for taking package information from 
file with raw package RPM headers (aka hdlist). 
...
=item * B<hdrl> - file with raw package headers in RPM format;
 ...
 this format isn't optimal for poldek. See poldekrc.sample-redhat-hdlist 
 included in poldek distribution.
