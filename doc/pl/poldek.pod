...
Ten dokument dotyczy serii 0.18.x i jest w du¿ej czê¶ci zdezaktualizowany, 
aczkolwiek nadal mo¿e byæ przdatny dla u¿ytkowników nie znaj±cych angielskiego. 

Aktualna dokumentacja jest dostêpna w jêzyku angielskim, w postaci stron 
podrêcznika systemowego man ('poldek' oraz 'poldek.conf') oraz w postaci 
podrêcznika w formacie Info oraz HTML.
...
Je¶li ¼ród³o jest skonfigurowane z nazw± (patrz L</PLIK KONFIGURACYJNY>),
wtedy mo¿e byæ wybrane przez B<-n, --sn> B<NAZWA-¬RÓD£A>, np.:
...
Opcja ta weryfikuje zale¿no¶ci zbioru pakietów.
...
Priorytety pakietów s± brane z PLIKU podanego przez --split-conf (patrz przyk³adowy
pkgsplit.conf z dystrybucji programu). Domy¶lnie pakiety maj± priorytet 0.

Ka¿da czê¶æ jest zapisywana jako lista NAME-EVR.ARCH.rpm do pliku
...
Wypisz nazwy plików pakietów do PLIKU b±d¼ na standardowe wyj¶cie, zamias instalowaæ je.

=item B<--dumpn[=PLIK]>

Wypisz nazwy pakietów do PLIKU b±d¼ na standardowe wyj¶cie, zamiast instalowaæ je.
...
Automatycznie aktualizuj pakiety, których zale¿no¶ci s± ³amane poprzez
...
Zapobiega aktualizacji podanych pakietów, je¶li s± ju¿ zainstalowane.
Je¶li opcja nie jest podana, wtedy lista pakietów do zatrzymania jest brana
z opcji konfiguracyjnej B<hold> i dodatkowo z $HOME/.poldek_hold.
Wstrzymakie pakiety nie bêd± aktualizowane za wyj±tkiem sytuacji, gdy bêd±
bezpo¶rednio wskazane przez u¿ytkownika.

=item B<-m, --mercy>

B±d¼ tolerancyjny w stosunku do zale¿no¶ci od wersji, które toleruje RPM, np.:
...
=item B<--rpmdef="NAZWA WARTO¦Æ">

Jak opcja rpm --define.

=item B<-t, --test>

Ñie instaluj pakietów, ale sprawd¼, czy zadzia³a czy nie. Podane raz
powoduje tylko test z u¿yciem poldka, ¿eby przeprowadziæ pe³ny test
...
KATALOG wskazuje miejsce, gdzie pobrane pliki maj± byæ przechowywane.
...
przez binarkê RPM, która jest uruchamiana w tle. Opcje przekazywane do rpma:
...
=item * --test (musi byæ podane dwukrotnie, patrz wy¿ej opis --test)
...
Inne opcje rpma mog± byæ przekazywane jako argumenty w postaci
...
Program próbuje przeczytaæ konfiguracjê z F<$HOME/.poldekrc>; je¶li plik ten nie
istnieje, czyta F</etc/poldek.conf>. Opcja B<-c,--conf> mo¿e wskazywaæ na inn±
lokalizacjê pliku konfiguracyjnego. Z B<--noconf> poldek nie czyta ¿adnej konfiguracji.

Plik konfiguracyjny zawiera parametry w nastêpuj±cej postaci:

'nazwa = warto¶æ'

Plik jest oparty na liniach, czyli ka¿da linia zakoñczona
...
Opcjonalna B<NAZWA> mo¿e byæ okre¶lona pó¼niej z linii poleceñ przez
B<-n, --sn> B<NAZWA-¬RÓD£A>; na przyk³ad je¶li masz ¼ród³a skonfigurowane
nastêpuj±co:
...
wtedy aby u¿yæ tylko ftp://zn/PLD/i686/PLD/RPMS wykonaj:

$ poldek --sn pld-stable

...i aby u¿yæ wszystkich pld-*:

$ poldek --sn pld-\*

B<OPCJE> okre¶laj± opcje ¼róde³; dozwolone warto¶ci to:

=over

=item * B<noauto> - nie ³aduj domy¶lnie tych ¼róde³

=item * B<noautoup> - nie aktualizuj domy¶lnie tych ¼róde³

=item * B<gpg> - weryfikuj sygnatury GPG pakietu

=item * B<pgp> - weryfikuj sygnatury PGP pakietu

=item * B<pri=WARTO¦Æ_NUMERYCZNA>

Opcj± t± mo¿na ustawiæ priorytet ¼ród³a. Je¶li te same pakiety
(identyczna nazwa i EVR) s± dostêpne z wiêcej ni¿ jednego ¼ród³a,
wtedy brane s± pakiety ze ¼ród³a o wy¿szym priorytecie.

Je¶li priorytety ¼róde³ nie s± ustawione, ¼ród³a dostaj± priorytety
...
=item * B<hdrl> - plik z surowymi nag³ówkami pakietów w formacie RPM;
 dostêpny w wielu dystrybucjach opartych na RPM-ie (w RH nazywa siê hdlist),
...
Je¶li chcesz skonfigurowaæ ¼ród³o z prefiksem (patrz opis opcji B<-P>),
wtedy powinno byæ okre¶lone jako zakoñczona sufiksem para ¼ród³o/prefiks,
aby rozró¿niæ dane ¼ród³o od innych, na przyk³ad:
...
=item * B<confirm_installation = yes|no>
        ...
        pakietów (domy¶lnie: no)

=item * B<confirm_removal = yes|no>

        Je¶li ustawione na "yes" program potwierdza wszystkie deinstalacje
        pakietów (domy¶lnie: yes)

=item * B<choose_equivalents_manually = yes|no>

        Je¶li ustawione na "yes" program pozwala u¿ytkownikowi na wybór
        spo¶ród równowa¿nych pakietów (domy¶lnie: no)
...
=item * B<keep_downloads = yes|no>
        ...
        Program domy¶lnie usuwa pobrane pliki pakietów po pomy¶lnej instalacji.
...
=item * B<particle_install = yes|no>
        ...
        Je¶li ustawione na "no" instalacja wielu pakietów jest przeprowadzana
...
SPEC jest poleceniem b±d¼ sformatowanym ³añcuchem, gdzie przynajmniej 2 z 4
dostêpnych semi-makr powinny byæ u¿yte:

=over

=item B<%p[n]> - nazwa pliku pakietu, %pn znaczy, ¿e %p mo¿e wyst±piæ wiele razy
...
Obecnie dostêpne s± cztery protoko³y: FTP, HTTP, HTTPS i RSYNC.  Specjalny
"protokó³" B<ftp_http> mo¿e byæ u¿yty do opisu programów pobieraj±cych z obu
protoko³ów, HTTP i FTP.
...
Dodatkowo poldek wspiera URL-e CDROM, które mog± byæ u¿ywane, gdy zestaw
pakietów jest przechowywany na zmienialnych mediach (kilka CD, ZIP-ów, etc);
jako CDROM "downloader" powinien byæ skonfigurowany vfjuggle, po szczegó³y
...
W PLD I<http://www.pld.org.pl>, które jest podstawow± platform± rozwojow±
dla poldka, tag "Obsoletes" jest u¿ywany w dwóch ró¿nych znaczeniach:
pierwsze oznacza B<A> I<uniewa¿nia> B<B>, drugie za¶ B<A> I<jest równowa¿ne> B<B>.
Oczywi¶cie jest niemo¿liwe, aby je rozró¿niæ i u¿yæ podczas aktualizacji systemu
(w Red Hat Linux u¿yte jest jedynie pierwsze znaczenie, wiêc instalator Red Hata
u¿ywa tego taga).
...
bazy danych RPM-a i b±d¼ uwa¿ny. Pamiêtaj, ¿e I<NIE MA ¯ADNEJ GWARANCJI>
i udanej zabawy.
