...
The poldek is the RPM package management tool. Its
basic features are similar to every modern package updater like
I<apt-get> -- program takes information about packages from
package index file (I<packages.dir.gz> by default) and allows
to package install, upgrade and removal.

Program supports two modes: a batch mode (in use similar
to I<apt-get> one) and I<interactive> one. The interactive
mode puts the user into a readline interface with command line
auto-completion and history, similar to shell mode of Perl's CPAN module.

Remote files are retrieved from remote servers by poldek
itself (internal FTP/HTTP client) or by external downloaders
(not configured by default, see L</CONFIG FILE>).

The poldek basic modes are:
...
=head1 PACKAGE SOURCES

Package sources may be specified in config file by B<source> parameter
(see L</CONFIG FILE>), or from cmdline by following options:
...
where SOURCE is a local path or URL. With B<--source> native package
index file (packages.dir.gz) is used; with B<--sdir> desired directory
is scanned instead of reading index file, should be used for local
directories only. The last one B<--shdrl> can be used for taking package
information from file with raw package RPM headers (aka hdlist).

Type of the source can be set by B<type> source option also,
see L</CONFIG FILE>.
...
To list all configured sources use B<-l, --sl> option: e.g:
...
If EVR must be set then pick it after '#', e.g:

$ poldek ... apache#1.3.12

$ poldek ... apache#1.3.20-1

=item * as package set defined in FILE B<--pset=FILE>,
...
Install, upgrade and downgrade given packages.
...
by uninstalled ones, for example if package foo is upgraded
from version 1.0 to 1.2 and package foo-devel is already installed
with requirement "foo = 1.0" then poldek upgrade foo-devel to version
1.2 too.
...
Source isn't loaded by default. 

=item * B<noautoup> 

Source index file is not resynchronized by default.

=item * B<gpg> 

With rpm <= 4.0.x GPG signature of package is verified before installation; 
with rpm >= 4.1 this option is equivalent of 'sign' (see below) 

=item * B<pgp> 

With rpm <= 4.0.x PGP signature of packages is verified before installation;
with rpm >= 4.1 this option is equivalent of 'sign' (see below) 

=item * B<sign> 

With rpm >= 4.1 all presented signatures of package are verified; with 
rpm <= 4.0.x option is equivalent of 'gpg'; 
...
packages from highest prioritized source are taken. Lower number means
higher priority, and negative priorities are allowed. Priority defaults 
to 0.

If several sources have the same priority (including default priority
of 0), they are prioritized in order they appear in config file, first
listed source has highest priority.

=item * B<type=SOURCE_TYPE>

Source type may be set by this option. Possible values are:

=over 2

=item * B<pidx> - native poldek's index file (the default)

=item * B<pdir> - for local directories, with it poldek
                  scans entire directory instead of reading index file.
...
$PAGER (if set) is used in interactive mode.
...
=head2 Upgrade more than one instance of the package
...
=head2 Homepage: I<http://team.pld.org.pl/~mis/poldek/>

=head2 Mailing list I<poldek@pld-linux.org>

Please send comments, bug reports, etc to the author or mailing list
